...
matrix solveUpper(matrix A, matrix U){// div/2 algo 
	...
	matrix L00=cilk_spawn solveUpper(A00,U00);
	matrix L10=cilk_spawn solveUpper(A10,U00);
	cilk_sync;
	
	matrix iA01=cilk_spawn subtract(A01,multiply(L00,U01));
	matrix iA11=cilk_spawn subtract(A11,multiply(L10,U01));
	cilk_sync;
	
	matrix L01=cilk_spawn solveUpper(iA01,U11); 
	matrix L11=cilk_spawn solveUpper(iA11,U11); 
	cilk_sync;
